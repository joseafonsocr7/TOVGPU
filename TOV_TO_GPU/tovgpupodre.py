...
class TOVGPU:
  ...
  def __init__(self, en_arr, p_arr, add_crust=True, plot_eos=False):
    ...
    self.en_dens = PchipInterpolator((p_arr[sort_ind]),
                                     (en_arr[sort_ind])) 

    sort_ind = np.argsort(en_arr)
    self.press = PchipInterpolator((en_arr[sort_ind]),
                                   (p_arr[sort_ind]))
  ...
  def add_crust(self):
    ...
    baym_eos = np.genfromtxt(crust_loc + "Baym_eos.dat", 
                             ...
                             names=["en", "p", "nB",])
    P_crust = PchipInterpolator(baym_eos["en"], baym_eos["p"])
    ...
    self.en_dens = PchipInterpolator(p_arr, en_arr)
    self.press = PchipInterpolator(en_arr,p_arr)
  ...
  def solve(self, c_dens, rmax=50e5, rtol=1.0e-5, dmrel=10e-12, dr=100):
    ...
    m = 4.0 * pi * r[0] ** 3 * eden
    kkk= m
    psol = odeint(self.tov_eq, [P.get(), m.get()], r.get(), rtol=rtol)
  ...
  def solve_tidal(self, c_dens, rmax=30e5, rtol=1.0e-4, dmrel=10e-12, dr=100):
    ...
    e_R = PchipInterpolator(r, e_R)
    p_R = PchipInterpolator(r, p_R)
    m_R = PchipInterpolator(r, m_R)

    beta0 = 2 * r[0]
    H0 = r[0] ** 2

    solution = odeint(self.love_eq, [beta0.get(), H0.get()], r.get(), args=([e_R, p_R, m_R],), rtol=rtol)
    beta = solution[-1, 0]
    H = solution[-1, 1]
